...
Call `groundmeas.db.connect_db(path)` once per process before using database-backed functions.
...
| `create_measurement(data)` | Insert measurement (optional nested `location`). Returns `measurement_id`. | `data`: measurement fields |
| `create_item(data, measurement_id)` | Insert item linked to a measurement. Returns `item_id`. | `measurement_id`: parent ID |
| `read_measurements(where=None)` | Raw SQL WHERE string; returns `(records, ids)` with items + location. | `where`: string |
| `read_measurements_by(**filters)` | Filtered read; supports `__lt/__lte/__gt/__gte/__in/__ne`. | e.g., `asset_type="substation"` |
| `read_items_by(**filters)` | Filtered read for items; same suffix rules. | e.g., `frequency_hz__gte=1000` |
...
| `_compute_magnitude` | Auto-computes missing polar/rectangular parts; raises `ValueError` if neither representation is present. | SQLAlchemy event |
...
| `distance_profile_value(measurement_id, measurement_type="earthing_impedance", algorithm="maximum", window=3)` | Reduce distance–value profile via chosen algorithm. | Algorithms: `maximum`, `62_percent`, `minimum_gradient`, `minimum_stddev`, `inverse` (fits relation below). |
...
| Function | Description | Key args/notes |
| --- | --- | --- |
| `rho_f_model(measurement_ids)` | Fit rho–f coefficients `(k1..k5)` using impedance (real/imag) + soil resistivity. | Requires overlapping freq points. |
...
| Function | Description | Key args/notes |
| --- | --- | --- |
| `voltage_vt_epr(measurement_ids, frequency=50)` | EPR and touch/prospective voltages per ampere. | Needs `earthing_impedance` + `earthing_current` at `frequency`. |
...
| `_current_item_to_complex(item)` | Convert item to complex current (A). | Prefers real/imag, else magnitude/angle. |
...
All return `plotly.graph_objects.Figure` for dashboards/notebooks.
...
| `build_items_from_rows(measurement_id, rows, measurement_type, frequency_hz, distance_to_current_injection_m=None)` | Assemble payloads for impedance, earthing currents (median if stable), prospective touch voltage near 1 m. | |
...
## Extension (`groundmeas.analytics_extension`)
| Function | Description | Key args/notes |
| --- | --- | --- |
| `value_over_distance(...)` | Duplicate of `analytics.value_over_distance`; currently unused elsewhere. | |
